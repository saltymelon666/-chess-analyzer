...
def thought_path_summary(
    ...
    global_posture = bool(
        global_posture_claim
        and global_posture_claim.statement in core
    )
    key_choice = bool(
        comparison_claim
        and comparison_claim.statement in core
        and (
            global_posture_claim is None
            or core.index(global_posture_claim.statement)
            < core.index(comparison_claim.statement)
        )
    )
    teaching_summary = "teaching_rule" not in kinds
    return {

...
- 完整 15 局真实 Stockfish + DeepSeek 套件为首次通过 `15/15`、最终严格通过 `15/15`、安全回退 `0/15`、核心命题严格落地 `15/15`、已声明命题逐字落地 `34/34`、缓存响应 4ms。固定 15 局没有命中新因果类型，因此新路径的实际内容由截图同型确定性样例覆盖，仍需部署后用用户原棋谱复跑确认。

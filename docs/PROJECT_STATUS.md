...
## 59. 2026-09-24 本机初筛后局面复核的登录凭证修复

- 排查线上分析时发现，生产页面调用 `/api/game-review/position` 时只带 `Content-Type`，漏带登录凭证；生产后端启用收费鉴权，该接口对无凭证请求返回 401。因此本机初筛虽可完成，用户点开棋步后无法取得后台权威复核，后续专业解释也无法正常生成。
- 开发页和 GitHub Pages 发布页的局面复核请求统一使用已有 `authorizedHeaders`，继续由后端按当前用户校验棋局归属。新增前端回归断言覆盖这条请求；未修改 Stockfish、Facts、Threat、Plan、DeepSeek、会员判断或密钥配置。
- 本次修复位于隔离工作树 `codex/fix-position-verification-auth`。完整回归 `422 passed, 1 warning`；两份页面 SHA-256 相同、内联 JavaScript 语法通过，`git diff --check` 通过。上线及用户原棋谱的棋理复核仍待确认；勿将本地单元测试等同于线上用户实际棋局验证。

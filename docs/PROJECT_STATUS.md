...
- 命题版本升至 `narrative_claims 1.7`，专业缓存版本升至 `professional-v47-multi-ply-reply-causality`。非吃子首应现在可以沿合法 Stockfish 路线追踪同一枚棋子：只有该子后来亲自吃掉首应后新增攻击且始终未移动/未被替换的目标时，才说明调动如何兑现。正文会明确写出 `Ne4` 将白马从 c3 转到 e4、瞄住 c5 黑兵，经过 `Bf5、Re1、Bg7` 后由同一匹马以 `Nxc5` 兑现，并明确这一具体收获不能单独解释全部 95cp 评价变化。

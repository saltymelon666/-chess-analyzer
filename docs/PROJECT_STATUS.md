...
- 局面切换、去重、失败兜底和过期恢复 Node 专项 `4/4` 通过；引擎专项 `11/11` 通过；本地真实 API 短棋谱返回 200、2 步、基础深度 20，随后专业分析返回 200 且有正文。完整回归 `385 passed, 3 failed, 1 warning`，3 项仍是此前记录的公测文案、`chess.js` 加载方式和开局展示断言漂移，本轮未新增失败。两份页面 SHA-256 一致、内联 JavaScript 语法和 `git diff --check` 通过。部署与线上同源验收待完成。

...
## 42. 2026-09-15 整盘评价瞬时故障自动恢复

- 生产复现确认 `/api/health` 持续返回 200 并不能代表整盘评价可用：一次最短合法棋谱请求实际收到 Render `502 Bad Gateway`，而单局面 Stockfish 请求正常；稍后同一整盘接口恢复。16 个半回合真实棋谱首次计算约 176.9 秒并成功返回约 1.97 MB 结果，说明整盘任务会长时间占用单一 Stockfish 通道，原网页只重试 3 次且仅处理 503，遇到 502、504、429、短暂断网或持续排队会过早失败。
- 前端整盘评价现在把 429、502、503、504 和非超时网络中断纳入最多 12 次的有界退避重试，单次完整分析保留 21 分钟上限；等待期间明确显示“服务器正在恢复或排队”或“连接中断”，用户无需手动连续点击。`AbortError` 和非瞬时 4xx 仍直接结束，不隐藏真实输入错误。
- 根目录与 `docs/index.html` 同步，内联 JavaScript 语法、SHA-256 一致性及新增两项重试专项测试通过。完整回归为 `382 passed, 5 failed, 1 warning`：3 项是主分支已有的公测文案、`chess.js` 加载方式与开局上下文断言漂移；另 2 项仅因隔离发布工作树没有根目录 `stockfish.exe`，不是代码回归。
- PR #59 已合并为 `b5eb530`，GitHub Pages 部署成功；`pawnlab.cn` 已读取到新重试逻辑。部署后 16 个半回合真实整盘请求再次以 200 完成，耗时 176.9 秒、返回约 1.97 MB。此次验证证明新页面和整盘接口已上线，但没有用户失败棋谱或浏览器错误原文，因此该特定棋谱是否另有 PGN 格式问题仍待按原样复现。

## 41. 2026-09-15 全局高深度 Stockfish 与逐局面解释恢复

- 整盘所有局面由基础深度 10 提升到深度 20、MultiPV 3；候选分差、相邻评价跳变或将杀信号达到稳定性条件时，最多选 12 个关键局面复核到深度 22，首选或分数仍发生实质变化时确认到深度 24。低深度专项调用仍按相对增量复核，避免辅助路径从深度 10 直接跳到 22。
- Render Web Service 配置由 free 调整为 starter，Stockfish Hash 从 64MB 提升到 256MB，单局面超时 90 秒、整盘服务端上限 1200 秒；浏览器整盘等待上限同步为 1260 秒，并明确提示长棋谱可能需要 3—20 分钟。未增加线程数，避免在低核数实例上因线程争用降低稳定性。
- 局面解释以 `analysis_id + move_index + opening revision` 隔离缓存；每次切换都会使旧响应失效，快速拖动只请求停稳后的局面，相同请求合并。详细解释失败或缓存过期时不再空白，而是立即展示由整盘 Stockfish 事实包生成的走法质量、评价变化、首选着和实战后核验路线，并提供重试或重新评价入口。
- 本机对同一盘棋抽取 8 个局面做真实 Stockfish 深度 10/20 对照：深度 10 用时 0.24 秒，深度 20 用时 40.23 秒；8 个局面中 3 个首选着改变，最大评分差 192cp、平均差 58.3cp，证明原深度 10 不足以作为与 Lichess 对照的稳定口径。该耗时只代表本机，不是 Render 线上 SLA。
- 局面切换、去重、失败兜底和过期恢复 Node 专项 `4/4` 通过；引擎专项 `11/11` 通过；本地真实 API 短棋谱返回 200、2 步、基础深度 20，随后专业分析返回 200 且有正文。完整回归 `385 passed, 3 failed, 1 warning`，3 项仍是此前记录的公测文案、`chess.js` 加载方式和开局展示断言漂移，本轮未新增失败。两份页面 SHA-256 一致、内联 JavaScript 语法和 `git diff --check` 通过。
- PR #57 已合并为 `95093d2`，GitHub Pages 构建与发布成功；生产页面已包含请求去重、引擎兜底和 1260 秒浏览器等待配置。Render 自动部署后，从 `https://pawnlab.cn` 来源提交 2 半回合真实棋谱返回 200、2 步、基础深度 20、耗时约 41 秒；随后对第 1、2 个局面分别请求专业分析，均返回 200、正文存在、无警告，确认不是只生成单一局面解释。

## 40. 2026-09-14 多步调动的实战因果修复

- 纠正上一轮验收错误：v46 只覆盖“非吃子首应新增攻击重要子力、对手下一手立即处理”的窄路径，且生产验收使用了另一盘 `Bf5 → g4 → Bd7`，不能证明用户截图中的 `Bxe5` 已修复。截图真实路线为 `Ne4 → Bf5 → Re1 → Bg7 → Nxc5 → Qc4 → Be3 → Rad8 → d6 → Bxb2`，因此 v46 仍退化为只报告评分、首选与首应。
- 命题版本升至 `narrative_claims 1.7`，专业缓存版本升至 `professional-v47-multi-ply-reply-causality`。非吃子首应现在可以沿合法 Stockfish 路线追踪同一枚棋子：只有该子后来亲自吃掉首应后新增攻击且始终未移动/未被替换的目标时，才说明调动如何兑现。截图同型夹具同时满足黑方首选 `Qxd6` 和完整十手路线；正文会明确写出 `Ne4` 将白马从 d6 转到 e4、瞄住 c5 黑兵，经过 `Bf5、Re1、Bg7` 后由同一匹马以 `Nxc5` 兑现，并明确这一具体收获不能单独解释全部 95cp 评价变化。
- 新增合法构造的同路线正例，以及另一匹同类棋子完成吃子、原棋子先被吃、原目标先移动三类反例；不会因为棋子类型相同或目标格后来重新出现棋子而误报同一因果链。
- 专业分析与命题专项 `135 passed`，引擎、整盘分析与质量集 `36 passed`。完整回归 `383 passed, 3 failed, 1 warning`，3 项仍是主分支已有的公测文案、`chess.js` 懒加载和开局上下文旧断言；`git diff --check` 通过。
- PR #54 已合并为 `5333a9e`，回归夹具对齐补充 PR #55 已合并为 `71ad321`；GitHub Pages 构建成功，Render 重启后旧分析缓存返回 404，确认新后端部署已切换。生产固定 15 局 Stockfish + DeepSeek 验收为 `15/15`：整盘与专业接口均返回 200、最终正文完整、校验警告 `0/15`；13 局首次通过，2 局经一次模型校正后通过。
- 本地 15 局真实 DeepSeek 套件因当前环境未配置 API Key 而未启动，改由生产环境完成上述 15 局验证。用户截图没有包含 FEN/PGN，因此目前只完成了同一着法序列的合法局面回归；在取得原棋谱并于生产复跑前，不把用户原局面标记为已验证。

## 39. 2026-09-14 非吃子应手的具体因果说明

- 修复“实战着只显示分数、引擎首选和对手首应，却不解释首应为何有效”的缺口。`narrative_claims` 升至 `1.6`、专业缓存版本升至 `professional-v46-reply-pressure-causality`。
- 新增严格的非吃子压力命题：仅当实战着至少损失 50cp、合法 Stockfish 实战路线的首应不是吃子、`python-chess` 确认该应手新增攻击马/象/车/后，并且主变下一手确实移动被攻击棋子回应时，才把两步写成因果链。正文会说明应手棋子的起止格、被攻击的重要子力以及实际回应，解释“带攻击的主动节奏”的具体来源。
- 截图同型确定性样例使用 `Bxe5 → Ne4 → Qe7`：白马从 c3 到 e4 后新增攻击 d6 黑后，黑方随后以 Qe7 把后移开；最终正文不再停在“白方首选响应 Ne4”，而会说明白方在调马同时让黑方先处理后的安全。反例把第二步改为与受攻后无关的 `...Kg8`，程序拒绝生成节奏因果，只保留已验证首应。
- 因果正文仍由程序验证命题逐字重建，没有放松未知引用、路线外走法、颜色、格子或额外因果校验。新增两项正反回归；专业分析、命题和决策上下文专项 `108 passed`，Stockfish 质量集 `17 passed`。
- 完整 15 局真实 Stockfish + DeepSeek 套件为首次通过 `15/15`、最终严格通过 `15/15`、安全回退 `0/15`、核心命题严格落地 `15/15`、已声明命题逐字落地 `34/34`、缓存响应 4ms。固定 15 局没有命中新因果类型，因此新路径另用构造正反例和真实大师棋谱覆盖。
- 完整回归为 `378 passed, 4 failed, 1 warning`：3项仍是主分支已有的公测文案、`chess.js`懒加载和开局上下文旧断言；第4项仅因隔离工作树缺少被忽略的 `stockfish.exe`，链接同一引擎后该测试单独 `1 passed`。`git diff --check` 通过。
- PR #52 已合并为 `d33e4f1`，对应 GitHub Pages 构建与发布成功。Render 生产接口随后用 Alapin—Alekhine（1911）完整 132 半回合棋谱验收：第22步 `11...Bf5` 为 `+0.86 → +1.76 / 90 cp / 不精确`，首选 `Re8`；专业正文明确写出 `g4` 使白兵从 g2 到 g4 并攻击 f5 黑象、黑方随后 `Bd7` 退象，以及白方借此取得带攻击的主动节奏。专业接口返回 200、非缓存、无校验警告，确认后端已经运行 v46 新逻辑。

## 38. 2026-09-14 实战评分同源与战略选择说明

- 截图中的 `f4` 暴露两个缺口：专业结论把页面已有的 `+1.24 → +1.05 / 19 cp / 好棋` 模糊改写为“评价接近”，视觉上像两套评分；同时没有解释封闭中心下 `f4` 与首选 `Qb3` 在计划和次序上的实际差别。
- 命题版本升至 `narrative_claims 1.5`、提示词缓存版本升至 `professional-v45-strategic-choice`。评价比较直接复用同一 `MoveReview` 的走前评价、走后评价、centipawn loss 和走法等级，不另算、不改写；前端有完整验证正文时不再把泛化的 `evaluationReason` 重复放在正文前面。
- 新增严格的 `verified_choice`：仅当 `python-chess` 确认封闭中心、实战着是翼侧兵推进，且合法 Stockfish 实战路线确认对手以中心兵吃子、己方兵回吃时，才说明“翼侧扩张—对手中心转换—兵型变化—后续组织—首选次序”。若首选路线稍后仍走实战着，会明确判为执行次序差异，而不是进攻方向错误。
- 截图同型确定性回归使用 `f4 → fxe5 → dxe5 → Qe7 → Qa5 → Nf8 → Rd3 → Rb8 → Rc3 → Kf7 → Nd4`，同时验证首选 `Qb3` 后稍晚再走 `f4`；生成正文逐字包含同源评分、中心兵型转换、`Qa5/Rd3/Rc3` 后续组织和次序结论。专业分析专项 `124 passed`，内联 JavaScript 语法、两份首页 SHA-256 和 `git diff --check` 均通过。
- PR #50 已合并到 `main`，生产提交为 `ebc3722e`；Render 已返回 v45 新正文。线上真实 `f4` 样本的整盘结果为 `+0.32 → +0.20 / 12 cp / 最佳着`，专业正文逐字使用同一组数值和等级，且无警告。完整回归为 `377 passed, 3 failed, 1 warning`，3 项仍是主线已有的公测文案、`chess.js` 懒加载和开局上下文旧断言，本轮没有新增失败。
- 部署后 15 局 Stockfish + DeepSeek 套件为 `15/15`：同源评分完整 `15/15`、过程话语清除 `15/15`、无效移动细节清除 `15/15`、必需引用完整 `15/15`、结论顺序正确 `15/15`、无警告 `15/15`；已有具体后果链继续保持 `4/4`。被忽略的可读报告位于 `work/book-consequence-v44/production-report.md`，不提交仓库。

## 37. 2026-09-14 棋书式实战着因果与后果说明

- 专业分析命题升至 `narrative_claims 1.4`、提示词缓存版本升至 `professional-v44-book-consequence`。实战着核心正文不再罗列棋子从哪走到哪、保护关系或控制格数，而是按“局面结论 → 实战选择的价值或问题 → 对手如何利用 → 具体机制 → 路线结果”组织。
- 新增程序验证的 `verified_consequence` 命题。系统只从合法 Stockfish 路线和 `python-chess` 棋盘重放中提取连续吃子、物质收支与路线清空机制；能够区分净丢子、等价交换、暂时吃子后被追回和真正保留的收益，不让 DeepSeek 自行补造棋盘事实。
- 页面每一手均显示“实战着分析”和“分析结论”；有路线时继续展示“验证路线”，结果栏改为“路线验证到的结果”，并清除“算到这里轮到某方走”等截断话语。根目录与 `docs/` 发布首页 SHA-256 一致，两份内联 JavaScript 语法通过。
- 专业分析相关专项为 `131 passed, 1 failed, 1 warning`，唯一失败是主线已有的开局上下文旧断言；完整回归为 `375 passed, 3 failed, 1 warning`，三项失败仍是已有的公测文案乱码断言、`chess.js` 懒加载断言和开局上下文断言，本轮没有新增失败。`git diff --check` 通过。
- PR #48 已合并到 `main`，生产提交为 `c7186da1`；对应 GitHub Pages 构建成功，`https://pawnlab.cn` 已出现“分析结论”和“路线验证到的结果”，旧“最终战术结果”已移除。生产健康接口返回 Stockfish 可用、DeepSeek `deepseek-v4-flash` 已配置；从 `https://pawnlab.cn` 来源提交真实 6 半回合棋谱后，整盘分析返回 6 步、深度 10，专业分析返回 200、无警告，核心正文没有棋子移动或控制格复述。
- 部署后的 15 局 Stockfish + DeepSeek 生产套件为 `15/15`：过程话语清除 `15/15`、无效移动细节清除 `15/15`、必需引用完整 `15/15`、结论顺序正确 `15/15`、无警告 `15/15`；其中 4 局触发已验证后果命题，完整 `4/4`。可读报告位于被忽略的 `work/book-consequence-v44/production-report.md`，不提交仓库。

## 36. 2026-09-13 局面优势因果解释与无效过程文案清理

- Stockfish候选路线卡已在PR #43中收窄为路线编号、首着、评价和完整着法序列；删除“第一步作用”“对手回应”及“按验证顺序列出”等无解释价值的占位文字，动态内容为空时不再硬填说明。
- 核心局面解读不再强制加入“王附近有几枚棋子”这类孤立统计、教学检查清单、无事件的首个回应，以及“评价差距不足以支持”“验证路线”“下一次遇到”等分析过程话语。小分差只用于区分“局面原有问题”和“本步新增代价”，不能代替对全局优劣原因的解释。
- 新增严格的“牵制后得子”证据链：程序从实战后FEN重放同一条合法Stockfish主变，使用python-chess确认绝对牵制，要求同一枚被牵制的马、象、车或后一直留在对应格，且后来在仍受牵制时被吃；正文才允许写出形成牵制的SAN、中间应对、吃子SAN和具体子力结果。只有牵制而没有后续吃子时不生成得子结论；已经存在的绝对牵制与紧接着的吃子也可识别。
- PR #44合并提交`6012437c`引入因果命题与新正文路径；PR #45合并提交`7d6ee102`修正小分差在占优、均衡和劣势局面中的语义错配；PR #46合并提交`fe8571db`升级命题版本并使旧缓存失效。GitHub Pages三次构建均成功，生产健康接口确认Stockfish可用、DeepSeek已配置，线上冒烟已返回修正后的占优语句且不再包含teaching claim。
- 牵制、无吃子反例、已有牵制立即吃子和三种小分差语境均有确定性回归；相关专项`121 passed`。完整回归为`372 passed, 3 failed, 1 warning`，3项仍是既有的公测文案、`chess.js`懒加载和开局上下文前端断言漂移，本轮没有新增失败。
- 修正后生产15局Stockfish + DeepSeek套件为`15/15`：过程话语清除`15/15`、teaching claim移除`15/15`、必需引用完整`15/15`、无警告`15/15`。固定15局中没有“绝对牵制后吃同一枚子”的样本，因此该战术链的线上真实局面覆盖仍需用用户原棋谱复跑确认；程序级合法性和防误报已由专项测试覆盖。可读报告位于`work/causal-v43/production-report.md`（工作产物，不提交仓库）。

## 35. 2026-09-12 整盘分析性能优化

- 保持生产整盘初筛深度 10 和 MultiPV 3，不用直接降深度换速度。同一份 40 半回合棋谱本机对比中，深度 8 虽由 2.301 秒降至 1.716 秒，但 41 个局面有 20 个首选着与深度 10 不同，不能作为安全加速方案。
- Stockfish 服务新增最多 2,048 个局面的进程内 LRU 结果缓存，键包含完整 FEN 和请求深度，只复用同一引擎配置下的已验证事实；重复提交同一棋谱、常见开局重合或自动重试时不再重复计算已有局面。11 局面本机基准由首次 0.718 秒降至缓存命中 0.003 秒，并避免再次启动引擎进程。
- 低深度稳定性复核仍按将杀信号、候选路线分差和相邻局面评价跳变选择关键位置，但每盘最多深查优先级最高的 12 个局面；普通位置继续保留深度 10 的三条路线，关键位置仍确认至深度 16，首选或评价实质变化时继续确认至深度 20。
- 新增缓存复用、缓存命中不启动引擎、深查数量上限和优先选择最不稳定局面的回归测试；引擎与整盘分析专项 `19 passed`。完整回归 `305 passed, 3 failed, 1 warning`，3 项仍为主线既有前端断言漂移，本轮没有新增失败。
- PR #34 已合并为 `46973d41` 并完成 Render 自动部署。线上用一盘此前未提交的 12 半回合棋谱验证：首次完整分析 5.30 秒返回 200，立即重复提交 1.67 秒返回 200；另一盘 20 半回合棋谱的缓存命中为 2.22—2.79 秒。缓存只加速相同局面，服务重启后的第一盘和全新中后盘仍需正常计算。

## 34. 2026-09-12 网页整盘分析繁忙恢复

- 线上复查确认首页、运行时配置、健康接口和 CORS 都正常，但真实 `/api/game-review` 连续在约 10 秒返回 503；问题是 Stockfish 单通道被长任务占用，不是网页域名或 API 连接中断。
- 后端排队窗口从 10 秒调整为 30 秒。整盘计算超时或请求取消时会向批处理线程发出停止信号，线程在当前局面结束后不再继续剩余局面与稳定性复核，从而更快退出并释放单通道锁。
- 首页整盘请求加入 285 秒浏览器超时与最多两次自动繁忙重试，遵循服务端 `Retry-After`；加载区明确提示整盘分析可能需要 1—3 分钟、服务器繁忙后的重试进度和最终等待超时，不再长期停在含糊的“正在准备每步评价”。开发页与发布页保持同步。
- 没有实际获得 Stockfish 通道的繁忙请求会从分析日志中丢弃，不再因前端自动重试产生多条失败记录。引擎、Analytics、API 和后台/首页专项测试 `36 passed`；完整回归 `303 passed, 3 failed, 1 warning`，3 项仍为主线既有前端断言漂移，本轮没有新增失败。
- PR #32 已合并为 `c7ca1da2` 并完成 GitHub Pages 与 Render 自动部署，未包含原工作区尚未提交的专业分析研究改动。线上首页已包含自动重试与 285 秒超时逻辑；随后从 `https://pawnlab.cn` 来源真实请求 6 半回合棋谱，8.7 秒返回 200 和完整逐步评价，健康接口与 CORS 同时正常。

## 33. 2026-09-11 后端整盘分析排队与状态修复

- Stockfish 仍保持单通道执行，但等待通道新增 10 秒上限；服务繁忙时 `/api/game-review` 返回可重试的 503 和 `Retry-After: 10`，不再让新请求无限等待并停在“正在准备每步评价”。
- Python 线程超时后可能继续执行，后端现在会在后台线程真正结束前继续占用单通道锁，避免超时请求释放锁后并发启动额外 Stockfish 进程、进一步压垮服务。
- 未完成的请求在历史记录中显示为“运行中”，并从成功率分母和失败数中排除；专业分析和独立报告的 DeepSeek 失败也不再把已经成功的整盘 Stockfish 分析覆盖成失败。后台整盘成功率现在只反映 `/api/game-review` 的最终结果。
- 引擎、Analytics、API 和后台状态专项测试 `33 passed`。完整回归 `300 passed, 3 failed, 1 warning`；3 项失败仍是主线既有的公测文案、`chess.js` 加载方式和开局上下文前端断言漂移，本轮没有新增失败。
- PR #30 已合并为 `75f539c1` 并完成 GitHub Pages 与 Render 自动部署，未包含原工作区尚未提交的专业分析研究改动。线上用同一份 40 半回合棋谱并发验收：首请求 27.9 秒返回 200，第二请求 10.8 秒返回 503、`Retry-After: 10` 和明确繁忙提示；随后健康检查继续返回 200。

## 32. 2026-08-28 运营后台全历史统计与独立评分反馈

- 运营后台移除按日期筛选，主指标、转化漏斗、Token、成本、分析日志和反馈明细统一读取 Analytics 数据库保留的全部时间范围；页面明确标注“全部历史统计”和“分析历史记录”。
- `GET /api/admin/dashboard` 新增 `historical_statistics` 和 `feedback_summary`。原 `statistics` 按日结构与 `GET /api/admin/statistics` 继续保留，避免破坏既有 API 调用；后台页面只使用全历史数据。
- 评分与反馈从分析日志中独立展示，增加反馈记录数、评分数量、平均评分、文字反馈数量四项汇总，并继续逐条显示评分、文字建议和可展开的关联分析结果。分析历史与反馈历史不再受所选日期限制，单次最多返回最近 1,000 条跨日期记录。
- Analytics/API/后台专项测试 `24 passed`；完整测试 `295 passed, 3 failed, 1 warning`，3 项失败仍为主线既有公测文案、`chess.js` 加载方式和开局上下文断言漂移，本轮没有新增失败。两个后台页面内容一致，内联 JavaScript 语法通过。
- Edge 无头浏览器用跨日期模拟数据完成桌面 `1365×900` 与手机 `390×844` 检查：均显示 10 个全历史指标、4 个评分反馈汇总和 2 条反馈明细，日期输入为 0，无脚本错误或横向溢出。
...
- 用户棋谱在 `10.Ne5` 后暴露固定深度 10 的主变波动：浅层整盘搜索曾暂时把黑方 `10...Nxd4` 评为 `-0.91`，但深度 12 以上首选翻转为 `10...Bb7`，深度 22—24 收敛至接近均势。问题来自低深度 MultiPV 未收敛，不是 DeepSeek 改写分数。
- 整盘分析现在对候选分差至少 50cp、相邻局面评价跳变至少 50cp 或涉及将杀的低深度结果自动复核至至少深度 16；若首选或评价仍发生实质变化，再确认至至少深度 20。稳定局面继续使用快速初筛。
- 引擎按 Stockfish 返回的 `multipv` 保存和排序路线，并强制要求 `rank=1` 存在；首选路线校验失败时明确终止，禁止把次选静默升级为最佳路线。
- 用户完整 98 半回合棋谱真实回归得到 `10.Ne5` 前后约 `-0.04 → -0.17`，黑方首选与候选路线第一项均为 `10...Bb7`，整盘耗时约 15.7 秒。正式 15 局 Stockfish + DeepSeek 部署前审核首次通过 `15/15`、最终严格通过 `15/15`、安全回退 `0/15`、事实引用精度 `100%`；旧深度 10 金标有两项随稳定路线变化而漂移，未作为事实错误处理。
- PR #26 已合并为 `c9f6b08` 并完成自动部署。GitHub Pages 构建成功；生产 `https://api.pawnlab.cn/api/health` 返回 200，Stockfish 与 DeepSeek 均可用。生产棋谱冒烟中 `10.Ne5` 后为 `-0.19`，黑方首选、候选路线 1 和专业分析路线 1 均为 `10...Bb7`，路线深度 20，原 `-0.91 / 10...Nxd4` 问题未再出现。

## 27. 2026-08-21 平面棋盘、居中棋子与复习页面

- 首页棋盘与棋子统一为无立体阴影的平面视觉：新增精确格线的 `flat_chess_board_v2.webp`，并切换到透明轮廓居中的 `pieces-v3/` 全套 12 枚棋子素材。
- 棋子显示尺寸缩进至单格内部，字母与数字坐标移到棋盘外框；12 枚素材的非透明轮廓中心误差均不超过 `0.5px`，避免棋子视觉偏斜或压到格线。
- 新增独立 `review.html` 复习训练页与首页复习入口；开发版和 `docs/` 发布版页面、棋盘与棋子资源保持同步。
- 本轮只合并前端棋盘、棋子、复习页及对应测试，不包含工作区内尚未提交的后端改动和研究资料删除。

## 26. 2026-08-20 首页分析工作区 UI 微调发布候选

- 在不修改 PGN、棋盘、Stockfish、DeepSeek、反馈、缓存和 API 行为的前提下，微调首页棋盘工作区与完整专业分析报告的视觉层级。
- 棋手信息、PGN 摘要、回放控制、整盘分析按钮和服务状态改为更紧凑的工具化布局；进度条使用浅黄色 `#F2D27A`。
- 专业分析正文继续完整展示，不新增折叠；局面、计划、走法质量和研究路线使用稳定语义样式，不改变事实来源或校验规则。
- 开发版与发布版前端规范化内容一致；内联 JavaScript 语法通过。Edge 桌面 1365px、平板 820px、手机 390px 检查无横向溢出，回放按钮具有可访问名称。
- 基于生产分支的完整回归为 `290 passed, 3 failed`；3 项均为既有前端断言漂移，没有新增功能失败。
...
## 24. 2026-08-19 首页反馈系统

- 首页底部新增与现有视觉一致的反馈区，支持 1—5 分评分、最多 2,000 字建议和提交状态提示；手机端自动切为单列布局。
- 反馈通过现有匿名 Analytics 事件入口提交，仅保存匿名访客标识、评分、建议和页面路径；后端兼容已有 SQLite/Postgres 表结构。
- 根目录 `index.html` 与 `docs/index.html` 保持一致；反馈专项测试 `22 passed`，内联 JavaScript 语法通过。完整 pytest 为 `288 passed, 5 failed`：3 项为 `origin/main` 既有前端断言差异，2 项因隔离发布工作树没有复制本地 Stockfish 可执行文件而失败。

## 25. 2026-08-19 反馈关联分析结果

- 用户提交反馈时，如当前正在查看专业分析，会一并发送分析 ID、当前着法上下文和页面上可见的分析正文；未进行分析时仍可提交普通反馈。
- 运营后台新增独立“用户反馈”区域，用户评价、用户建议和对应分析结果分开显示，长分析正文按需展开。
- SQLite 与 PostgreSQL 事件表通过兼容迁移新增 `analysis_result` 字段；后台接口按所选日期返回最近反馈。

## 26. 2026-09-12 v41 已验证专业棋评生产部署

- PR #35 已合并到 `main`，合并提交为 `125d45580bd8c70a65a9fa2144b475545536fcd4`；Render 自动部署完成。生产 OpenAPI 已包含 `claimRefs`，`/api/health` 返回 200，Stockfish 可用且 DeepSeek 已配置。
- 专业分析提示词升至 `professional-v41-forced-thought-path`。核心正文由程序按“全局态势 → 实战选择 → 关键比较或转折 → 棋理启示”重建；全局判断、实战着、评价比较、对手回应与教学原则分别受 Stockfish、python-chess 和已验证命题约束，DeepSeek 只选择讲解重点。
- 次佳着分析区分评价损失、首应立即吃子和路线后段事件；没有证据时不把相关事件写成全部分差的单一原因。仅在 `python-chess` 判定走后局面结束时使用终局表述。
- 新增只读统一棋书知识库、棋书评价顺序、近期决策上下文和原子命题质量指标；棋书资料只用于组织方式和解释方法，不覆盖当前棋盘事实。
- 15局真实 Stockfish + DeepSeek 套件最终严格通过15/15、安全回退0/15、89/89条核心声明逐字落地、强制思考路径15/15；实际模型为 `deepseek-v4-flash`。
- 最新生产主线部署候选完整pytest为 `366 passed, 3 failed, 1 warning`；3项失败仍是既有前端页面与旧断言漂移，本次部署没有修改前端。SQLite完整性检查为 `ok`，`git diff --check`通过。
- 生产实测使用一盘2个半回合的短棋谱：`/api/game-review` 返回200、2步和深度10；随后 `/api/professional-analysis` 返回200且无警告，包含6个合法 `claimRefs`，正文完整出现全局态势、关键选择和棋理启示，没有暴露UCI内部着法。
- 本次样本的核心命题观测越界为0，不代表总体幻觉概率为0；全页面事实准确率和经典棋书文风仍需更大规模GM盲评。

## 27. 2026-09-12 v42 自然棋书叙述生产部署

- PR #38 将专业分析升级为 `professional-v42-natural-chess-book-prose`，合并提交为 `d4afa0ff6d0ce1d58337ba0ecfaede3860786d43`；PR #39 修复自然正文重建后的最终长度收束，合并提交为 `3e9a43c3a29fe1631877d0ed3c15b0e8f6a85d72`。两次 Render 自动部署均已完成。
- 页面核心正文不再显示“先看全局”“实战把选择摆上棋盘”“再看关键选择”“从计划看”“这段变化留给初学者的原则是”等固定栏目口号；这些旧词已进入自动质量门。程序仍强制保持全局态势、实战选择、引擎比较或惩罚、棋理启示的因果顺序。
- 实战着、首选一致、次佳着分差、对手最强回应和教学检查次序改为连贯棋书语气；文案变化不改变 Stockfish、python-chess 和 `evidenceRefs` 的事实所有权，也不允许把路线后段事件倒推成第一回应的直接效果。
- 第一轮生产 15 局验证发现 `opening-3` 的安全结果在最终正文重建后达到 1794 字，超过普通档 1650 字上限；定位后在三条生成路径的事实正文重建之后重新执行非结构化内容收束。本地 Stockfish 复现由 1 个长度错误降为 0，生产单局复测返回 200、无警告。
- 修复后的第二轮生产 Stockfish + DeepSeek 固定 15 局套件：完整通过15/15、旧栏目词清除15/15、必需命题引用覆盖15/15、强制思考顺序15/15、无警告15/15；生产模型为 `deepseek-v4-flash`。
- 专业分析相关测试 `112 passed, 1 warning`；最终完整 pytest 为 `368 passed, 3 failed, 1 warning`。3项失败仍是既有前端页面与旧断言漂移，本次没有修改前端；`git diff --check`通过。

## 28. 2026-09-12 恢复页面局面解读正文

- 用户生产截图暴露出前后端展示断层：v42 已在 `playedMoveAnalysis.intention` 生成并校验自然棋评，但前端结论模式只显示实战验证路线和最终结果，核心局面解读没有渲染。
- PR #41 在局面概况之后、计划与验证路线之前新增“♟ 局面解读”卡片，完整显示后端验证正文；没有恢复自由文本、原始 JSON 或未经证据约束的字段。
- 根目录 `index.html` 与 `docs/index.html` 保持一致；新增正文展示、页面同步和不截断检查 `3 passed`，内联 JavaScript 语法通过，Edge DOM 实际渲染确认核心正文存在。
- 完整 pytest 为 `369 passed, 3 failed, 1 warning`；3项仍是已有前端旧断言，本次没有新增失败。生产 `https://pawnlab.cn/` 返回200，并已确认包含正文渲染逻辑、“♟ 局面解读”标题和对应样式。

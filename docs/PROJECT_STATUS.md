...
## 62. 2026-09-24 网页初筛中断恢复与文字缺失定位（本地审核版）

- 用户澄清“没有文字分析”是整盘评价一直卡在重试，并非已有逐步评价后局面解读面板为空。该流程必须先完成初筛或后台整盘评价；失败重试期间不会生成所选局面的权威复核及 DeepSeek 文本。
- 浏览器 Stockfish Worker 在扫描途中报错、单局面超时或返回无有效走法时，现在只重启一次 Worker 并重扫当前局面，保留此前已完成的局面结果；不再因一次瞬时故障立即放弃整盘初筛。持续失败仍走原后台路径，不伪造引擎分数或文字。后台回退状态将展示本机错误原因和实际 HTTP 状态码，便于定位当前浏览器故障。
- 开发/发布页面完全一致，内联脚本语法检查通过；新增模拟 Worker 瞬时与持续失败的运行时测试 `2/2` 通过，完整 pytest `425 passed, 1 warning`。本地 8080 页面刷新后可取到修改版；用户随后反馈“可以了”并明确要求上线，但没有提供逐步计时或文字内容截图，不能据此量化本机性能或证明棋理解读质量。
- PR #69 已合并为 `a08884c`，GitHub Pages 部署任务 `35977066238` 成功。实时访问 `https://pawnlab.cn/index.html` 返回200且包含 Worker 重启、逐局面进度及 HTTP 错误提示；Stockfish JS/WASM 资源均返回200，线上 API 健康状态为 `ok`。本次只发布前端与测试/文档，未修改后端。正式站用户会话的整盘分析与文字解读仍需刷新页面后实际验收，以上静态与健康检查不能代替此验证。

## 61. 2026-09-24 网页长棋分析重试状态诊断（本地待验收）

- 用户的网页截图显示“分析服务器正在恢复或排队，20 秒后自动重试（4/11）”，证明此次没有走完正常的浏览器初筛与轻量任务建立流程，而是进入后台整盘分析回退，并至少发生多次可重试请求失败；截图本身未给出具体 HTTP 状态码或浏览器 Worker 失败原因，根因仍待当前浏览器复现。
- 本地网页把浏览器扫描的 `X/Y` 进度直接显示在分析按钮上；后台任务建立、切换后台及重试次数也同步显示在按钮上，避免用户只看到固定“正在评价整盘棋”。重试说明改为显示实际 HTTP 状态码，并在开发者控制台记录本机扫描异常，以便区分引擎失败与服务器限流/网关错误。没有调整 Stockfish 深度、计费、权限或复核事实来源。
- 当前 8080 本地预览会在页面刷新后提供这版前端；生产站尚未部署。两份页面 SHA-256 一致，内联 JavaScript 语法检查通过，`git diff --check` 通过；修改前完整回归为 `424 passed, 1 warning`。用户浏览器中的具体 Worker 错误、实际重试状态码以及这盘完整分析是否成功，均尚未验证，不能以 `/api/health` 的 200 代替。

## 60. 2026-09-24 本地网页分析接口地址修复

- 用户明确反馈问题发生在网页，当前打开的是 `http://127.0.0.1:8080/index.html?view=app`。以同一 PGN 在该网页实际准备登录与分析时，页面虽然通过 `runtime-config.js` 配置了线上 API，却因 `API_BASE_URL` 的 localhost 强制分支转向未运行的 `127.0.0.1:8000`；因此本地网页无法连接账号和分析服务。单独验证线上 API 不足以证明这条网页路径可用。
- 开发/发布页调整为优先采用明确配置的 API origin；只有配置为空时，本地预览才回落到 `127.0.0.1:8000`。这与两份 `runtime-config.js` 已有注释一致。新增前端断言覆盖优先级；不改变后端权限、棋盘事实或分析算法。
- 当前本地预览服务已从包含修复的隔离工作树在8080端口运行，页面及 Stockfish JS/WASM 资源均返回200，线上 API 对本地 origin 的预检返回200并允许跨域。
- 修复 API 地址后，以独立 Edge 真正点击本地网页按钮并跑完用户原 PGN：本机初筛约34秒完成89步；页面随后自动对导入时默认选中的最后一步发起深度20复核，约第143秒才完成，DeepSeek约第159秒生成结论，页面无报错。这个自动复核不是用户点选，造成“点击开始分析后一直等”的主要体感。已改为初筛完成立即显示逐步结果，只有用户点击棋谱中的一步才触发该步后台复核与解释；未删减后台正式深度。
- 最终以同一 PGN 从本地网页按钮复测：第36秒完成89步初筛且显示“选择棋步后生成”，最后一步没有自动复核；模拟用户点击第45步后，第77秒返回后台权威复核，第89秒出现完整教练解释，页面无报错，测试会话注销返回204。完整回归 `424 passed, 1 warning`；两份页面哈希一致、内联 JavaScript 语法通过；独立 Edge 检查桌面1440、平板768、手机390宽度均无横向溢出或页面脚本异常。PR #67 已合并为 `2d3acef`，GitHub Pages 部署成功；实时读取 `https://pawnlab.cn/index.html` 确认已包含配置的 API 地址优先级与点选后才正式复核的代码。生产网站的用户本人会话仍需用户刷新后验收。

## 59. 2026-09-24 整盘初筛进度被隐藏的诊断与修复

- 用户报告点击开始分析后“一直扫描”，并提供了89半回合 PGN。以同一棋谱在独立本机 Edge 运行浏览器 Stockfish 初筛，90个局面均达到深度12，修复前约31秒、修复后约22秒；这是本机测量，不代表所有设备或浏览器的速度。
...
- PR #61 已合并为生产提交 `bc045a8`，PR #62 的空提交 `eadbd9c` 用于重新触发 Render 部署；GitHub Pages 与 Render 最终均已切换到新版本。正式上线包含 Chessire 式介绍首页、手机号账号、1 盘免费完整分析、单层月/年会员、微信/支付宝个人收款码、人工付款审核、历史教练记忆、Played Move vs Best Move、长棋 Prompt 压缩、浏览器 Stockfish 初筛与后台按需权威复核，同时保留生产深度20、关键局面22/24复核与12次瞬时故障重试。
- 首次 Render 部署失败并非代码构建问题：生产 PostgreSQL `pawnlab-analytics` 的免费实例已到期暂停，启动时无法解析原内部数据库主机。用户明确确认后，将数据库升级为 Render 最低付费计算套餐 `0.1c-256mb`（$6/月）；数据库恢复后，手动部署 `eadbd9c` 成功，服务状态为 Live。不得再将该数据库降回会在30天后到期的免费实例。
- 上线前合并回归为 `421 passed, 1 warning`；15局真实 Stockfish + DeepSeek 最终严格通过 `15/15`、安全回退 `0/15`。页面与发布副本哈希一致，`git diff --check` 通过，未提交 `.env`、管理员密钥或 API Key。
- 生产冒烟使用隔离测试账号完成：注册密码不一致返回422；注册后免费次数初始未使用；首次 `/api/game-review/start-local` 成功并在后端标记已使用，第二次返回402；第1步后台 Stockfish 权威复核在22.3秒返回 `authoritative=true`、最佳着及程序化走法对比；DeepSeek专业解释11.2秒返回、无警告；教练记忆写入1条，另一账号看不到该记忆或付款申请。
- 个人收款配置在线，微信与支付宝二维码资源均返回200；首月月会员人工付款申请以990分创建为 `pending`，本人可见、另一账号不可见；无管理员密钥访问审核接口返回401。生产管理员页面返回200并包含“确认到账并开通”，但本次没有读取或传输管理员密钥，因此未自动批准测试订单；运营后台应将手机号以199开头的本轮测试申请标记为拒绝。两个测试账号会保留在生产数据库，测试会话均已注销，临时令牌文件已删除。

## 57. 2026-09-19 浏览器整盘扫描与按需后台复核审核版

- 将仓库已有的 Stockfish 18 Lite WASM/Web Worker 正式接入整盘分析：每个用户的浏览器以深度12扫描全部局面并生成用于导航的初筛徽标，整盘扫描不再占用服务端唯一的 Stockfish 通道。若浏览器不支持或本机引擎超时，页面自动回退到原有 `/api/game-review` 后台完整分析，不影响现有可用链路。
- 新增 `/api/game-review/start-local`：后端合法解析并重放 PGN、执行账号权益原子占用、建立严格绑定当前 `user_id` 的棋局任务，但不接收也不信任浏览器评价、最佳着或PV。本机扫描完成仍视为一次完整分析；免费次数、会员判断和防前端绕过继续全部由后端执行。
- 新增 `/api/game-review/position`：用户点开任意一步时，后台从已验证PGN恢复该步，使用与原后台整盘分析相同的 Stockfish版本、正式深度、MultiPV和 `analyze_pgn` 构建逻辑，重新检查走前与走后局面。相同用户、棋局和步号的并发请求合并；结果标记 `authoritative=true` 并替换页面初筛结果。
- Facts、Threat、Plan、Played Move vs Best Move、专业解读、复习保存及教练记忆只读取后台复核后的 `MoveReview`。未复核的浏览器扫描数据请求专业接口会返回409，不能进入 DeepSeek Prompt；历史记忆只根据用户实际打开且经后台确认的局面增量保存，不会采信客户端事实或串用户。
- 页面明确显示“本机初筛 / 后台已确认”。浏览器扫描使用白方统一视角显示分数，但保持 UCI MultiPV 原始排名，修复黑方局面在视角换算后可能把较差候选误排为第一选择的问题。后台复核失败时保留初筛用于定位，同时明确不把它显示成最终教练结论。
- 本地真实接口验证：4个半回合的任务建立约59ms，点开第3步后台复核约1.26秒并返回 `authoritative=true`；WASM命令行验证资源报告 `Stockfish 18 Lite WASM`，深度12、MultiPV=3正常返回。当前本地默认后台整盘深度为10，部署环境仍以其 `GAME_ANALYSIS_DEPTH` 配置为准；按需复核直接复用同一配置，因此最终结果与该环境原后台分析一致。
- 新增浏览器任务归属、跨用户隔离、免费次数占用、选中局面后台复核和前端双层门禁测试。最终完整回归 `421 passed, 2 warnings`；两份页面字节级一致，内联JavaScript语法通过。15局真实 Stockfish + DeepSeek 套件首次及最终严格通过均为 `15/15`，安全回退 `0/15`，引用ID合法率 `100%`，人工金标关键原子召回 `89.7%`，缓存6ms，结果见 `work/hybrid-local-review-v1/`；自动门禁不等同于专家盲审。本轮仍为本地审核版，未提交、推送或部署，桌面/手机真实页面交互仍需用户审核。

## 56. 2026-09-17 手机号注册与密码二次确认审核版

- 新账号注册标识由邮箱改为中国大陆手机号，接受 11 位号码及 `+86` / `86` 前缀并统一归一化。账号表增量增加唯一手机号列；为兼容旧表的非空邮箱约束，新手机号账号仅在数据库内部写入不可见的兼容占位值，对外账号响应不暴露该值。
- 注册请求新增 `password_confirm`，前端在提交前检查两次密码，后端 Pydantic 模型再次强制检查；不一致时确认框标红、焦点回到确认密码并显示具体提示，绕过前端仍返回 422，不能创建账号。提交期间登录和注册按钮均禁用，避免重复请求。
- 登录默认使用手机号；现有邮箱账号仍可在同一输入框输入原邮箱登录，旧账号、会话、会员、免费次数和历史教练数据不迁移、不覆盖。登录后的账号栏优先显示手机号，旧账号继续显示原邮箱。
- 真实 Edge 检查桌面 1440×1050 与手机 390×844，手机号、密码、确认密码、兼容说明和不一致错误均完整可见，页面无横向溢出。账号、收费、前端及支付专项 `32 passed`，完整回归 `420 passed, 2 warnings`；本地后端重启后健康检查 200，密码不一致的真实接口请求返回 422。开发/发布页面已同步，仍为本地审核版，未提交、推送或部署。

## 55. 2026-09-17 个人收款码与手机号人工审核版

- 将用户提供的微信、支付宝个人收款码原样转换为无损 WebP，分别接入开发版与发布版 `assets/payment/`。支付页选择月会员或年会员后，可切换微信、支付宝并查看真实收款码；二维码按原始宽高比显示，不裁切、不拉伸。两张资源均通过二维码解码器可识别验证，未输出或保存二维码中的支付内容。
- 人工付款核对字段由登录邮箱改为付款人手机号。前端使用电话输入框，后端接受中国大陆 11 位手机号以及 `+86` / `86` 前缀并统一归一化；手机号只用于人工核对，会员申请仍严格绑定当前登录 `user_id`，不能凭手机号直接开通或串到其他用户。
- 后端新增并兼容迁移 `manual_payment_requests.phone`。旧数据库保留既有 `email` 列以满足历史表约束，新申请仅由当前登录账号的服务端记录填充该兼容列，API 和管理员页面不再暴露或要求付款邮箱；老库无需删表或重建即可增加手机号字段。
- 用户提交后只生成待审核订单，不会自行激活会员。管理员页面必须使用已配置的 `ADMIN_STATISTICS_KEY`，核对真实到账、金额、渠道和套餐后才能点击通过；通过立即开通申请绑定账号，拒绝不开放权益，重复通过保持幂等。个人收款码不能程序化确认到账，且可能受微信或支付宝经营收款规则限制，正式商业使用前仍需自行确认平台规则。
- 真实 Edge 检查桌面 1440×1050 与手机 390×844：微信、支付宝收款码、手机号输入和审核后台均正常显示，所有页面 `clientWidth == scrollWidth`，手机支付弹窗可滚动。收费、账号和支付专项 `30 passed`，完整回归 `418 passed, 2 warnings`；警告仍为既有 Starlette 弃用提示和 pytest 缓存目录权限。开发/发布产品页与管理员页已同步，内联 JavaScript 语法通过。本轮仍是本地审核版，未提交、推送或部署，必须等待用户审核和明确上线授权。

## 54. 2026-09-16 微信支付与支付宝本地审核版

- 在既有订单与会员状态机上增量接入微信支付 API v3 和支付宝开放平台：桌面微信使用 Native 二维码，手机微信使用 H5；支付宝按设备使用电脑网站支付或手机网站支付。前端现在先选择月/年套餐，再明确选择微信或支付宝，微信二维码由后端本地生成，不把支付链接发送给第三方二维码服务；支付页桌面和390px手机端均无横向溢出。
- 后端新增渠道下单、本人订单状态查询、微信异步通知和支付宝异步通知接口。微信请求使用商户 RSA 签名，平台响应与通知使用平台公钥验签，通知资源使用 API v3 Key 解密；支付宝请求与通知使用 RSA2。正式通知只有在 AppID/商户号、渠道、订单号、人民币金额、成功状态和平台签名全部匹配数据库订单后，才进入原有会员激活入口。
- 支付成功继续保持终态和幂等：重复成功通知不会重复延长会员，迟到失败不能降级已生效会员；渠道交易号建立唯一索引，订单状态只允许所属用户查询。生产环境关闭旧的内部 HMAC 测试回调，前端无法直接修改会员状态。老数据库通过增量增加支付渠道和过期时间字段，旧订单默认标记为内部渠道，不删除原记录。
- 新增 `app/payments.py`、支付渠道配置、部署环境变量占位与商户配置说明；私钥、API v3 Key 和平台公钥均不进入仓库。未完整配置的渠道明确返回503，不生成假付款地址。真实收款仍需用户在微信/支付宝商户后台开通 Native/H5/电脑网站/手机网站产品，并在部署环境填入正式商户参数后做官方沙箱或小额实付验收；本轮未连接真实商户、未扣款、未提交、推送或部署。
- 新增7项支付专项测试，覆盖微信通知验签与解密、支付宝RSA2、金额/渠道不符拒绝、双渠道成功即时生效、重复通知幂等、跨用户订单隔离、未配置渠道503和旧表增量迁移。完整回归 `413 passed, 2 warnings`；警告仍为既有 Starlette 弃用提示和 pytest 缓存目录权限。开发/发布页面 SHA-256 一致，`git diff --check` 无空白错误，本地前后端健康检查为 HTTP 200。真实 Edge CDP 检查桌面1440×1050和手机390×844支付页，均为 `clientWidth == scrollWidth`，手机弹窗可滚动且套餐、微信、支付宝入口完整可见。

## 53. 2026-09-16 首页、导入区与棋盘状态字可读性复审

- 根据后续截图反馈，分析台的任务说明与棋谱导入不再共用一个外层白色卡片：桌面窄宽度下左右分成两张独立卡片，宽屏侧栏中上下分开，手机端按任务顺序上下排列。导入按钮与清空按钮保持同一操作行但使用独立按钮边界，不再因侧栏宽度不足换行。
- 第二次复核明确移除“导入棋谱”区域自身的大型白色外卡片、边框和阴影，只保留独立标签、紧凑棋谱输入框及操作按钮；输入框高度由112px收敛至92px，避免导入区再次形成一整块大容器。
- 同步清除导入区域遗留的整体 hover 上浮、边框变色、重阴影和相关过渡；鼠标经过时区域保持原位，仅按钮继续提供必要的操作反馈。
- 按用户三张真实页面截图继续收敛本地审核版：首页与分析台任务标题移除 Smiley Sans 斜体展示感，统一改为 700 字重的中文系统正体，并下调首页标题和正文尺度，减少 AI 营销海报感。
- 首页三项能力由狭窄三栏改为“短标题 + 完整说明”的纵向信息行，避免中文标题和说明碎断；手机端继续单列自然换行。
- 分析台导入区重新平衡左右列宽，主按钮不再横向铺满整栏，清空操作保持明确的次级层级；手机端主按钮仍自适应占据可用宽度。
- 棋盘深绿容器顶部的局面与行棋方标签改为深墨绿文字、暖纸底和明确边框，字号与字重固定为 14px/700，解决浅色文字叠在浅色胶囊上看不清的问题。
- 本轮只修改开发/发布页面的视觉样式与设计规范，不改变 PGN、棋盘、Stockfish、DeepSeek、Facts、Threat、Plan、收费或账号逻辑；仍为本地审核版，未提交、推送或部署。
- 真实 Edge CDP 验证桌面 1440×1050 与手机 390×844：首页和分析台均为 `clientWidth == scrollWidth`，导入区立即可见；棋盘顶部标签计算样式为深墨绿 `rgb(23,60,54)`、暖纸底 `rgb(247,242,232)`、14px/700。最终完整回归 `406 passed, 2 warnings`，两份页面各2段内联脚本语法通过、SHA-256一致、设计JSON可解析、`git diff --check` 无空白错误，首页、分析台、审核台与后端健康接口均为 HTTP 200。本轮 Impeccable 检测因环境缺少 HTML parser 降级为正则扫描，报告包含大量既有设计系统漂移，不能作为无问题证明，因此以真实浏览器计算样式和截图作为本轮视觉验收依据。

## 52. 2026-09-15 连续教练、程序走法对比、长棋压缩与单层会员收费骨架

- 新增持久化账号、30天会话、后端分析权益与订单状态：新账号只有1次完整免费分析；免费权益在分析开始时原子占用，并发请求不能重复使用，失败会释放、成功才写入 `free_analysis_used`。月会员首单为9.9元、之后19.9元，年会员168元；会员仅在服务端校验通过的签名支付回调后立即生效，到期会失效。取消只停止续费状态，不提前收回已付款有效期。生产环境默认开启 `BILLING_ENFORCED`，旧分析与遥测表只做增量建表，不改写原数据。
- 新增最近5盘、严格按 `user_id` 隔离的教练记忆。每盘保存核心教训、实战重点、主要错误类型、关键局面与训练建议；生成新棋局专业解读时排除当前盘记录，只发送压缩历史、程序确认的重复问题与改善问题。Prompt明确这些信息不能覆盖当前 Stockfish、Facts、Threat、Plan 或程序走法对比。
- 每个 `MoveReview` 新增由 `python-chess` 合法重放与 Stockfish PV 生成的 `playedBestComparison`，只陈述程序确认的立即吃子、将军、直接受攻、保护、直接威胁及攻防关系变化。前端单独展示“你的走法 / 更好的走法 / 真正的区别”；模型不得自行补造漏吃、漏将或丢子事实。
- 长棋超过60个半回合时启用压缩：保留前10个完整回合、所有不准确/失误/严重失误及前后各1步、已有程序转折信号、关键PV和验证事实；其余连续普通着合并为回合区间摘要。短棋保留完整着法记录，普通着不重复携带PV与事实，避免反向放大Token。
- 新增注册/登录和单层会员付费页，月付与年付功能一致，年付标记“最划算”但不默认选择。后端新增商业事件及漏斗统计。支付采用渠道无关的“创建订单 + HMAC签名回调”接口；当前仓库未配置真实支付渠道与商户回调，因此只能验证订单和生效状态机，不能宣称已真实收款或正式上线。
- 收尾修复了两项既有前端回归：`chess.js` 恢复通过 `pgn-runtime.js` 按需加载，首屏使用固定初始FEN；专业结论恢复当前着、整盘摘要的开局回退链、开局版本缓存和完整开局说明。另修复账号状态、退出、下单和专业结论请求漏带登录令牌的问题。支付成功成为不可被迟到失败通知覆盖的终态，成功与失败重复回调均幂等；开局补充或教练记忆写入异常不再让已完成的Stockfish分析对用户返回失败。新增会员到期、续费顺延、回调乱序/幂等、辅助记忆故障及三个分析缓存接口跨用户隔离测试。
- 最终完整回归 `402 passed, 2 warnings`，无失败；最终收费、账号与压缩专项 `12 passed`，前一轮收费、账号和前端专项 `27 passed`。两个页面的内联JavaScript语法通过，根目录与 `docs/index.html` SHA-256一致，`git diff --check`通过。本地HTTP冒烟验证页面、懒加载模块、健康检查、注册、账号读取、首月990分订单、2个半回合真实Stockfish完整分析及免费次数扣除均成功。当前环境没有Playwright，未完成真实浏览器视觉截图验收。
- v44完整15局真实 Stockfish + DeepSeek：首次严格通过11/15，4局重试后通过，最终严格通过15/15，安全回退0/15，引用ID合法率100%，人工金标关键原子召回89.7%，缓存8ms；结果见 `work/continuity-comparison-compression-v44/`。自动门禁不等同于专家盲审，真实长棋Token节省和收费转化仍需在部署后按等价请求观察。
- 新增独立本地上线审核台 `work/release-review-v1/`，加载当前真实产品页并提供免费未使用、免费已用完、月会员、年会员四种隔离身份，月会员预置3盘教练记忆，并可载入72个半回合的合法长棋样例。审核台包含桌面/390px预览、12项验收清单、意见与JSON复制；数据只写入独立 `review.sqlite3`，支付地址为空，不扣款、不部署。最终390px截图确认长棋按钮文案可见、身份卡只在限定容器内横向滚动，独立视觉终审结论为 `ship`；审核包及收费专项回归15项通过，页面JavaScript、启动脚本与 `git diff --check` 通过。正式发布仍必须等待用户明确授权。
- 用户首轮审核结论为 `changes_requested`：账号、免费权益、定价、会员、事实同源、桌面/手机和范围控制通过；走法对比、教练记忆、长棋压缩及异常恢复仍待后续功能验收。按用户指定的 Chessire 式两段路径重做审核版：首页先介绍产品价值并展示 P1 研究员与 P2 侦探的组合主视觉，点击后才进入棋盘分析台，分析台提供返回首页入口。开发与发布页面、WebP 主视觉同步，真实390px检查为 `clientWidth=390`、`scrollWidth=390`；前端与审核包专项 `23 passed`，内联JavaScript、资源哈希和 `git diff --check` 通过。当前仍未提交、推送或部署。
- Chessire 式第二版完成最终视觉修正：剩余卡片统一为带模糊的柔和阴影，顶部栏不再单独入场，分析台仅保留一次整体切换；方向契约明确记录为用户钉选的 brief-pinned 两段路径。独立终审对三项修正均判定 `resolved`、无回归，最终 disposition 为 `ship`。新增根目录 `DESIGN.md` 与 `.impeccable/design.json`，固化暖纸、墨绿、行动黄、Smiley Sans、P1/P2 双角色和真实390px单列规则；结论仅代表本地审核版可交用户复审，仍未提交、推送或部署。
- 第二版最终完整回归为 `406 passed, 2 warnings`，警告仅为既有 Starlette 弃用提示与 pytest 缓存目录权限；`design.json` 可解析，开发/发布页面哈希一致，`git diff --check` 无错误，审核页、产品页与后端健康接口均返回 HTTP 200。
- 根据用户“AI感太重”的复审意见，使用 Impeccable quieter 收敛首页：关闭遗留点阵与光斑，页面背景固定为纯暖米白；移除主视觉与分析台吉祥物上的 P1/P2 可见文字标签和半透明说明条；降低首页标题尺度、圆角与阴影强度。真实 CDP 验证桌面与390px手机端无横向溢出，计算背景为 `rgb(244, 240, 231)`，可见正文不含 P1/P2 标签；独立收尾审查 disposition 为 `ship`。最终完整回归 `406 passed, 2 warnings`，页面哈希、内联脚本、设计JSON与本地HTTP检查通过。仍为本地审核版，未上线。
